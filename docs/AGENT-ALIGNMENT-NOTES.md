...
- Spec → Acceptance Criteria → Branch (`feat/<lane>/<slug>`) → Implementation → PR → Review (Copilot/Claude/Codex) → CI → squash-merge → deploy → Notion status update.
...
2. **`package.json`** — `packageManager` pnpm `9.12.0` → **`10.12.4`**, added `engines.node >=24.16.0`. **All app dependencies (Next/React/Tailwind/zod/eslint/TS) were left unchanged.**
