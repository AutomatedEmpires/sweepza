...
import { getHostBillingSnapshot, createStripePortalSession } from "@/lib/db/host-dashboard";
      ...
      <div className="mt-6 grid gap-3">
        <div className="rounded-2xl border border-ink/10 bg-white p-4">
          ...
          <p className="mt-1 text-xs text-ink/60">
            {snapshot.activeListingCount} of {snapshot.includedActiveListings} active listing slots used
          </p>
          {snapshot.isFull ? (
            <p className="mt-2 text-xs font-medium text-ink/70">Add another slot — ${snapshot.addSlotPriceMonthly}/mo</p>
          ) : null}
        </div>

        <form action={createStripePortalSession}>
          <button className="h-11 w-full rounded-xl border border-ink/15 bg-white text-sm font-semibold text-ink">
            Manage billing
          </button>
        </form>
      </div>
