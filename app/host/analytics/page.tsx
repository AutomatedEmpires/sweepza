...
export default async function HostAnalyticsPage() {
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-8">
      ...
      <h2 className="mb-3 mt-8 text-lg font-medium text-ink">Per listing</h2>
      {analytics.perListing.length === 0 ? (
        <p className="text-sm text-ink/60">No listing activity yet.</p>
      ) : (
        <div className="space-y-3">
          {analytics.perListing.map((row) => (
            <div key={row.listingId} className="rounded-card border border-sand bg-cream p-4 shadow-sm">
              <div className="flex items-center justify-between">
                <h3 className="font-medium text-ink">{row.title}</h3>
                <span className="text-sm text-ink/60">{row.conversionRatePct}% conversion</span>
              </div>
              <p className="mt-1 text-sm text-ink/60">{row.viewCount} views · {row.enterCount} entries</p>
            </div>
          ))}
        </div>
      )}
