...
export function AdminNav({ counts }: { counts: AdminNavCounts }) {
  ...
  return (
    <nav className="border-b border-sand md:w-60 md:shrink-0 md:border-b-0 md:border-r md:py-8">
        ...
        {items.map((item) => {
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              className={`flex shrink-0 items-center justify-between gap-2 rounded-full px-4 py-2 text-sm font-semibold transition md:rounded-xl ${
                active
                  ? "bg-ink text-cream"
            ...
            >
              ...
              {typeof item.badge === "number" && item.badge > 0 ? (
                <span className="inline-flex min-w-[1.25rem] items-center justify-center rounded-full bg-ember px-1.5 py-0.5 text-[11px] font-bold text-cream">
                  {item.badge}
                </span>
              ) : null}
            </Link>
