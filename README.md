...
| `/api/cron/seeker-reminders` | every minute | Claims one bounded, acknowledged seeker-scan wave and creates at most one durable digest per seeker/UTC day. It is database-free until both email gates are ready. |
| `/api/cron/email-deliveries` | every minute | Purges expired payloads and processes one bounded retry wave. Provider calls still require the separate literal `OUTBOUND_EMAIL_ENABLED="true"` gate. |
